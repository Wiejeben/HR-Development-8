...
export interface ITuple<a, b> {
    fst: a,
    snd: b,
};

export const Tuple = <a, b>(fst: a) => (snd: b): ITuple<a, b> => ({ fst: fst, snd: snd });
 ...
 */
export const splitAt = <a>(i: number) => (l: List<a>): ITuple<List<a>, List<a>> =>
    (l.kind === 'cons') ?
        (i <= 0) ? Tuple<List<a>, List<a>>(Cons(l.head)(Empty()))(l.tail) :
            ((t: ITuple<List<a>, List<a>>): ITuple<List<a>, List<a>> => Tuple<List<a>, List<a>>(Cons(l.head)(t.fst))(t.snd))(splitAt<a>(i - 1)(l.tail)) :
        undefined;
